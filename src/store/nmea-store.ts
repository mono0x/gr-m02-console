...
const initialPosition: PositionState = { lat: null, lon: null, altitude: null, geoidSep: null, updatedAt: 0 };
...
const initialMotion: MotionState = { speedKnots: null, speedKmh: null, courseTrue: null, courseMag: null };
  ...
  subscribeWithSelector((set) => ({
    ...
    ingestVTG: (data) => {
      set((state) => {
        ...
        const nextFix: FixState = { ...state.fix, modeIndicator: data.modeIndicator ?? state.fix.modeIndicator };
        const motionChanged = shallowDiff(state.motion, nextMotion);
        const fixChanged = shallowDiff(state.fix, nextFix);
      ...
      set((state) => {
        const nextPosition: PositionState =
          data.lat !== null && data.lon !== null
            ? { ...state.position, lat: data.lat, lon: data.lon, updatedAt: ts }
            : state.position;
        const nextFix: FixState = { ...state.fix, status: data.status, modeIndicator: data.modeIndicator };
        const positionChanged = nextPosition !== state.position && shallowDiff(state.position, nextPosition);
        const fixChanged = shallowDiff(state.fix, nextFix);
